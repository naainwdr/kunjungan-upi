...
async function processCode(code) {
    ...
    try {
        const res = await fetch(`{{ route('petugas.scanner.lookup') }}?kode=${encodeURIComponent(code)}`, {
...
function renderActions(data) {
    ...
    if (canIn) {
        area.innerHTML = `
        <form method="POST" action="/petugas/presensi/${currentKunjunganId}/checkin">
        ...
        area.innerHTML = `
        <form method="POST" action="/petugas/presensi/${currentKunjunganId}/checkout">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
