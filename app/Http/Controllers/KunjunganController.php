...
class KunjunganController extends Controller
{
    ...
    /**
     * Menampilkan form evaluasi kunjungan (legacy).
     *
     * @param  string $id Nomor registrasi kunjungan
     * @return View
     */
    public function evaluasiForm(string $id): View
    {
        // Ambil kunjungan dengan relasi yang dibutuhkan di view
        $kunjungan = Kunjungan::with(['sekolah', 'kontak'])
            ->where('nomor_registrasi', $id)
            ->firstOrFail();

        // Delegasikan validasi akses ke service — hindari logic di controller
        if (! $this->surveiService->isEvaluasiValid($kunjungan)) {
            // Tentukan pesan error yang spesifik berdasarkan kondisi
            if ($kunjungan->status !== 'completed') {
                abort(403, 'Formulir evaluasi hanya tersedia untuk kunjungan yang telah selesai.');
            }
            abort(403, 'Batas waktu pengisian evaluasi sudah lewat (7 hari setelah kunjungan selesai).');
        }
    ...
    /**
     * Menyimpan data evaluasi kunjungan (legacy).
     *
     * @param  SimpanEvaluasiRequest $request Request yang sudah tervalidasi
     * @param  string                $id      Nomor registrasi kunjungan
     * @return RedirectResponse
     */
    public function simpanEvaluasi(SimpanEvaluasiRequest $request, string $id): RedirectResponse
    {
        // Ambil kunjungan untuk diteruskan ke service
        $kunjungan = Kunjungan::where('nomor_registrasi', $id)->firstOrFail();

        // Validasi ulang akses form (defense in depth terhadap direct POST)
        if (! $this->surveiService->isEvaluasiValid($kunjungan)) {
            abort(403, 'Formulir evaluasi tidak dapat diakses saat ini.');
        }

        // Delegasikan penyimpanan ke service
        $this->surveiService->simpanEvaluasi($kunjungan, $request->validated());

        // Redirect ke halaman terima kasih setelah evaluasi tersimpan
        return redirect()->route('evaluasi.terima-kasih', $kunjungan->nomor_registrasi);
    }

    /**
     * Menampilkan halaman terima kasih setelah evaluasi berhasil disimpan.
     *
     * @param  string $id Nomor registrasi kunjungan
     * @return View
     */
    public function terimaKasih(string $id): View
    {
        // Ambil kunjungan untuk ditampilkan di halaman konfirmasi
        $kunjungan = Kunjungan::where('nomor_registrasi', $id)->firstOrFail();
